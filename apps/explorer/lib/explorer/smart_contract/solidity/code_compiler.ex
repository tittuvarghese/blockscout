defmodule Explorer.SmartContract.Solidity.CodeCompiler do
  ...
  @recuired_standard_input_fields ~w(language sources settings)
  ...
  defp tune_json(json_input) when is_binary(json_input) do
    case Jason.decode(json_input) do
      {:ok, map_input} ->
        ...
        map_set_recuired_keys = MapSet.new(@recuired_standard_input_fields)

        if MapSet.subset?(map_set_recuired_keys, map_set_input_keys) do
